					 ...
					 const struct ktermios *old)
{
	...
	if (port->dev_data->geni_se_set_rate) {
		ret = port->dev_data->geni_se_set_rate(&port->se, port->baud);
		if (ret) {
			dev_err(port->se.dev,
...
{
	struct qcom_geni_serial_port *port = to_dev_port(uport);
	struct device *pwr_dev = port->se.pd_list->pd_devs[DOMAIN_IDX_POWER];

	return geni_se_transition_d3d0(&port->se, pwr_dev, state);
}
