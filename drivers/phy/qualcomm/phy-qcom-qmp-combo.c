...
static int qmp_v456_configure_dp_phy(struct qmp_combo *qmp)
{
	...
	if (readl_poll_timeout(qmp->dp_serdes + cfg->regs[QPHY_COM_C_READY_STATUS],
			...
			500,
			10000)) {
		dev_err(qmp->dev, "QPHY_COM_C_READY_STATUS bit0 timedout\n");
		return -ETIMEDOUT;
	}
	...
	if (readl_poll_timeout(qmp->dp_serdes + cfg->regs[QPHY_COM_CMN_STATUS],
			...
			500,
			10000)) {
		dev_err(qmp->dev, "QPHY_COM_CMN_STATUS bit0 timedout\n");
		return -ETIMEDOUT;
	}
	...
	if (readl_poll_timeout(qmp->dp_serdes + cfg->regs[QPHY_COM_CMN_STATUS],
			...
			500,
			10000)) {
		dev_err(qmp->dev, "QPHY_COM_CMN_STATUS bit1 timedout\n");
		return -ETIMEDOUT;
	}
...
static int qmp_v8_configure_dp_phy(struct qmp_combo *qmp)
{
	...
	if (readl_poll_timeout(qmp->dp_dp_phy + cfg->regs[QPHY_DP_PHY_STATUS],
			...
			500,
			10000)) {
		dev_err(qmp->dev, "QPHY_DP_PHY_STATUS bit0 timedout\n");
		return -ETIMEDOUT;
	}
